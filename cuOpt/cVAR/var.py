...
##
def solve_var_portfolio(scenarios, scenario_probs, mu, alpha=0.95, lambda_risk=1.0,
                        w_min=None, w_max=None, solver_settings=None):
    """
    Solve VaR portfolio optimization using cuOpt linear programming.

    Parameters:
    - scenarios: numpy array of return scenarios (n_scenarios x n_assets)
    - scenario_probs: probability weights for scenarios
    - mu: expected returns vector
    - alpha: confidence level for VaR (default 0.95)
    ...
    - var_value: VaR value at optimum
    ...
    problem = Problem("var_portfolio_optimization")
    ...
    # Objective: maximize expected return - lambda * VaR
    # VaR = t + R_s^T w ≥ 0
    ...
    # Subtract VaR terms to penalize higher risk (lower VaR increases objective value)
    if lambda_risk != 0:
    ...
    # VaR constraints: t + R_s^T w ≥ 0 for all scenarios s
    for s in range(n_scenarios):
        var_constraint_expr = LinearExpression([], [], 0.0)
        var_constraint_expr += t     # + t

        # Add portfolio return terms: + R_s^T * w
        for i in range(n_assets):
            if scenarios[s, i] != 0:
                var_constraint_expr += w[i] * scenarios[s, i]
        problem.addConstraint(var_constraint_expr >= 0.0, name=f"var_{s}")
    print(f"the contratint expression is ****{var_constraint_expr}****")
    ...
    if problem.Status.name == "Optimal":
        ...
        # Calculate VaR and expected return
        var_value = t_value 
        expected_return = np.dot(mu, optimal_weights)
        print(f"the VAR value is ****** {t_value}**************")
        return optimal_weights, var_value, expected_return, problem
    else:
...
# Solve the optimization problem
try:
    optimal_weights, var_value, expected_return, solve_result = solve_var_portfolio(
...
print(f"Optimization method: VaR with cuOpt GPU acceleration")
